...
const ApartmentsBuy = ({ navigation }) => {
    ...
    <View style={{ backgroundColor: "#f6f8fc" }}>
      ...
      >
        ...
        <ScrollView
          ...
          contentContainerStyle={{ padding: 2, top: 0, paddingLeft: -5 }}
        >
          <View
            style={{
              flexDirection: "row",
        ...
        {books.length > 0 ? (
          <FlatList
            data={books.filter(
              (post) => post.status == "for sale" && post.category_id == "1"
            )}
            Vertical
